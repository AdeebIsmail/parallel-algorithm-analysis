// main.cpp — MPI Sample Sort with Caliper/Adiak instrumentation
// (C++14-compatible)
...
static void usage(const char *prog) {
  if (!prog)
    prog = "sample_sort";
  std::cerr << "Usage: " << prog
            << " [--n N] [--datatype int|double] [--input "
               "random|sorted|reversed|perturbed1] [--seed S]\n"
            << "Examples:\n"
            << "  mpirun -np 8 " << prog
            << " --n 1048576 --datatype int --input random\n";
}
...
static Options parse_cli(int argc, char **argv, int rank) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--n") {
      ...
      need(i);
      opt.group_num = std::atoi(argv[++i]);
    } else if (a == "--scalability") {
      need(i);
      opt.scalability = argv[++i];
    } else {
      if (!rank)
...
// 2) Ensure boundary monotonicity: last element of rank r-1 <= first element of
// rank r.
template <class T>
static bool global_sorted_check(const std::vector<T> &local, MPI_Comm comm) {
  ...
  MPI_Datatype Ttype =
      std::is_same<T, double>::value ? MPI_DOUBLE : MPI_LONG_LONG;

  T recv_prev_last = std::numeric_limits<T>::lowest();
  if (rank > 0) {
    T send_first = first;
    MPI_Sendrecv(&send_first, 1, Ttype, rank - 1, 11, &recv_prev_last, 1, Ttype,
                 rank - 1, 12, comm, MPI_STATUS_IGNORE);
  }
  T recv_next_first = std::numeric_limits<T>::max();
  if (rank + 1 < P) {
    T send_last = last;
    MPI_Sendrecv(&send_last, 1, Ttype, rank + 1, 12, &recv_next_first, 1, Ttype,
                 rank + 1, 11, comm, MPI_STATUS_IGNORE);
  }

  int boundary_ok = 1;
  if (rank > 0 && !local.empty()) {
    if (recv_prev_last > first)
      boundary_ok = 0;
  }
  if (rank + 1 < P && !local.empty()) {
    if (last > recv_next_first &&
        recv_next_first != std::numeric_limits<T>::max())
      boundary_ok = 0;
  }
  int gboundary_ok = 0;
...
// K-way merge of sorted runs concatenated in 'in' with run sizes in 'sizes'
// (sum = total). Produces sorted 'out'.
template <class T>
static void kway_merge(const std::vector<T> &in, const std::vector<int> &sizes,
                       std::vector<T> &out) {
...
static void sample_sort(std::vector<T> &local, MPI_Comm comm) {
  ...
  MPI_Allgatherv(samples.data(), loc_s, Ttype, all_samples.data(),
                 all_counts.data(), displs.data(), Ttype, comm);
  ...
  if (!all_samples.empty()) {
    for (int i = 1; i < P; ++i) {
      size_t idx = (size_t)std::llround(
          (long double)i * (long double)all_samples.size() / (long double)P);
      if (idx >= all_samples.size())
        idx = all_samples.size() - 1;
  ...
  if (!local.empty()) {
    ...
    bounds.reserve((size_t)P + 1);
    bounds.push_back(0);
    for (int i = 0; i < P - 1; ++i) {
      typename std::vector<T>::iterator it = std::upper_bound(
          local.begin() + bounds.back(), local.end(), splitters[i]);
  ...
  std::vector<T> sendbuf(local.size());
  {
    if (!local.empty()) {
      ...
      for (int i = 0; i < P; ++i) {
        ...
        std::copy(local.begin() + b, local.begin() + e,
                  sendbuf.begin() + send_displs[i]);
      }
    }
  }
  CALI_MARK_END("comp_small");
  ...
  CALI_MARK_BEGIN("comp");
  CALI_MARK_BEGIN("comp_large");
  std::vector<T> final_sorted;
  final_sorted.reserve(recvbuf.size());
...
template <> struct DataInit<int64_t> {
  static void fill(std::vector<int64_t> &v, const Options &opt, int rank) {
    ...
    if (opt.input == "random") {
      ...
      for (size_t i = 0; i < v.size(); ++i)
        v[i] = (int64_t)(rank) * 1000000000LL + (int64_t)i;
      ...
      for (size_t s = 0; s < swaps; ++s) {
        size_t a = gen() % v.size(), b = gen() % v.size();
        std::swap(v[a], v[b]);
      }
    } else {
      for (size_t i = 0; i < v.size(); ++i)
...
template <> struct DataInit<double> {
  static void fill(std::vector<double> &v, const Options &opt, int rank) {
    ...
    if (opt.input == "random") {
      ...
      for (size_t i = 0; i < v.size(); ++i)
        v[i] = (double)rank * 1e6 + (double)i;
    } else if (opt.input == "reversed") {
      for (size_t i = 0; i < v.size(); ++i)
        v[i] = -(double)((double)rank * 1e6 + (double)i);
    } else if (opt.input == "perturbed1") {
      for (size_t i = 0; i < v.size(); ++i)
        v[i] = (double)i;
      size_t swaps = v.size() / 100 + 1;
...
int main(int argc, char **argv) {
  ...
  adiak::value(
      "size_of_data_type",
      (int)(opt.datatype == "double" ? sizeof(double) : sizeof(int64_t)));
  ...
  if (!use_double)
    local_i64 = make_local_data<int64_t>(opt, comm);
  else
    local_d = make_local_data<double>(opt, comm);

  // Run sort
  if (!use_double)
    sample_sort<int64_t>(local_i64, comm);
  else
    sample_sort<double>(local_d, comm);

  // Correctness check (global)
  CALI_MARK_BEGIN("correctness_check");
  bool ok = false;
  if (!use_double)
    ok = global_sorted_check<int64_t>(local_i64, comm);
  else
    ok = global_sorted_check<double>(local_d, comm);
  CALI_MARK_END("correctness_check");

  if (!rank) {
    std::cout << "Global sorted check: " << (ok ? "PASSED" : "FAILED")
              << std::endl;
  }
